...
!(function() {
    ...
    module.exports = (function(window, document) {
        ...
        function MouseEventStream() {
            var self = this;
            var pointermove =
                'PointerEvent' in window ? 'pointermove' : 'MSPointerEvent' in window ? 'MSPointerMove' : 'mousemove';
        ...
        function MouseButtonEventStream() {
            var self = this;
            var pointerup =
                'PointerEvent' in window ? 'pointerup' : 'MSPointerEvent' in window ? 'MSPointerUp' : 'mouseup';
            var pointerdown =
                'PointerEvent' in window ? 'pointerdown' : 'MSPointerEvent' in window ? 'MSPointerDown' : 'mousedown';
        ...
        var cssCascade = {
            ...
            polyfillStyleInterface: function(cssPropertyName) {
                ...
                cssCascade.startMonitoringRule(
                    cssSyntax.parse('[style*="' + cssPropertyName + '"]{' + cssPropertyName + ':attr(style)}').value[0]
                );
                cssCascade.startMonitoringRule(
    ...
    module.exports = (function(window, document) {
        ...
        function currentStyleOf(element) {
            var style =
                element.cascadedStyle || element.specifiedStyle || element.currentStyle || getComputedStyle(element); // TODO: check CSSOM spec for real name
            if (!style.parentElement) {
                style.parentElement = element;
            }
            return style;
    ...
    module.exports = (function(window, document) {
        ...
        GridItem.prototype = {
            ...
            updateFromElement: function() {
                ...
                this.hMargins =
                    parseInt(usedStyle.getPropertyValue('margin-left')) +
                    parseInt(usedStyle.getPropertyValue('margin-right'));
                this.vMargins =
                    parseInt(usedStyle.getPropertyValue('margin-top')) +
                    parseInt(usedStyle.getPropertyValue('margin-bottom'));
                this.hPaddings =
                    parseInt(usedStyle.getPropertyValue('padding-left')) +
                    parseInt(usedStyle.getPropertyValue('padding-right'));
                this.vPaddings =
                    parseInt(usedStyle.getPropertyValue('padding-top')) +
                    parseInt(usedStyle.getPropertyValue('padding-bottom'));
                this.hBorders =
                    parseInt(usedStyle.getPropertyValue('border-left-width')) +
                    parseInt(usedStyle.getPropertyValue('border-right-width'));
                this.vBorders =
                    parseInt(usedStyle.getPropertyValue('border-top-width')) +
                    parseInt(usedStyle.getPropertyValue('border-bottom-width'));
                ...
                if (style['grid-area']) {
                    ...
                    this.parseLocationInstructions(
                        this.specifiedXStart,
                        this.specifiedXEnd,
                        col_start + ' / ' + col_end
                    );
                    this.parseLocationInstructions(
                        this.specifiedYStart,
                        this.specifiedYEnd,
                        row_start + ' / ' + row_end
                    );
                }
            ...
            parseLocationInstructions: function(specifiedStart, specifiedEnd, cssText) {
                ...
                var updateNameOrIndex = function(data) {
                    if (value[I] instanceof cssSyntax.IdentifierToken) {
                        ...
                        if (data.name) {
                            ...
                            console.error(
                                'INVALID DECLARATION: grid-column/row: ' +
                                    value.toCSSString() +
                                    ' (duplicate line name)'
                            );
                            this.buggy = true;
                        ...
                        if (value[I].value != data.index) {
                            console.error(
                                'INVALID DECLARATION: grid-column/row: ' + value.toCSSString() + ' (non-integer number)'
                            );
                            this.buggy = true;
                            return true;
                        }
                        ...
                        // this is wrong
                        console.error(
                            'INVALID DECLARATION: grid-column/row: ' + value.toCSSString() + ' (invalid token)'
                        );
                        this.buggy = true;
                        return true;
                    }
                ...
                if (!value[I]) {
                    console.error(
                        'INVALID DECLARATION: grid-column/row: ' + value.toCSSString() + ' (empty declaration)'
                    );
                    this.buggy = true;
                    return;
                }
                ...
                gridColumnStart: while (true) {
                    if (value[I] instanceof cssSyntax.IdentifierToken) {
                        if (value[I].value == 'span') {
                            if (!value[++I]) {
                                console.error(
                                    'INVALID DECLARATION: grid-column/row: ' +
                                        value.toCSSString() +
                                        ' (span is not a valid line name, more tokens expected)'
                                );
                                this.buggy = true;
                                return;
                            }
                        ...
                        console.error(
                            'INVALID DECLARATION: grid-column/row: ' +
                                value.toCSSString() +
                                ' (no token to analyze before the slash token)'
                        );
                        this.buggy = true;
                        return;
                    } else {
                    ...
                    if (value[I] instanceof cssSyntax.DelimToken && value[I].value == '/') {
                        ...
                        if (!value[++I]) {
                            ...
                            console.error(
                                'INVALID DECLARATION: grid-column/row: ' +
                                    value.toCSSString() +
                                    ' (expected at least one more token after the slash token)'
                            );
                            this.buggy = true;
                            return;
                        }
                ...
                if (value[I]) {
                    console.error(
                        'INVALID DECLARATION: grid-column/row: ' + value.toCSSString() + ' (tokens after end)'
                    );
                    this.buggy = true;
                    return;
                }
        ...
        GridLayout.prototype = {
            ...
            updateFromElement: function() {
                ...
                if ((cssText = style['grid-auto-flow'])) {
                    // FIXME: should be in a function
                ...
                if ((cssText = style['grid-row-gap'])) {
                    this.parseGridRowGap(cssText);
                }
                if ((cssText = style['grid-column-gap'])) {
                    this.parseGridColumnGap(cssText);
                }
                if ((cssText = style['grid-gap'])) {
                    this.parseGridGap(cssText);
                }
            ...
            parseTrackBreadthToken: function(cssToken) {
                ...
                if (cssToken instanceof cssSyntax.IdentifierToken) {
                    ...
                    if (cssToken.unit == 'fr') {
                        ...
                        return {
                            type: TRACK_BREADTH_LENGTH,
                            value: cssUnits.convertToPixels(cssToken.toCSSString(), this.element),
                        };
                    }
            ...
            parseTrackBreadth: function(value, I) {
                ...
                var parseTrackBreadthToken = function() {
                    ...
                    console.error(
                        'INVALID DECLARATION: grid-template-rows/columns: ' +
                            value.toCSSString() +
                            ' (unrecognized track breadth)'
                    );
                    buggy = true;
                    return;
                };
                ...
                if (value[I] instanceof cssSyntax.Func && value[I].name == 'minmax') {
                    ...
                    if (args.length != 2) {
                        console.error(
                            'INVALID DECLARATION: grid-template-rows/columns: ' +
                                value_backup.toCSSString() +
                                ' (invalid number of arguments to the minmax function)'
                        );
                        buggy = true;
                        return;
                    }
            ...
            parseGridTemplate: function(cssText) {
                // TODO: I used some lazy heuristics here
                var buggy = false;
                ...
                if (/"|'/.test(cssText)) {
                    ...
                    var strings = [];
                    cssText = cssText.replace(/\s*("(?:.*?)"|'(?:.*?)')\s*([-_a-zA-Z0-9]*)\s*/g, function(
                        data,
                        str,
                        size
                    ) {
                ...
                while (value[I]) {
                    ...
                    while (str !== '') {
                        ...
                        if (cell != '.' && cell[0] != '.') {
                            if (!areas[cell]) {
                                areas[cell] = {
                                    xStart: columns.length,
                                    xEnd: columns.length + 1,
                                    yStart: I - 1,
                                    yEnd: I,
                                };
                            }
            ...
            parseTrackDefinitions: function(lineNames, trackBreadths, cssText) {
                ...
                cssText = cssText
                    .replace(/\[/g, '(')
                    .replace(/\]/g, ')')
                    .replace(/repeat\(\s*([0-9]+)\s*\,((?:\([^()]*\)|[^()])+)\)/gi, function(s, n, v) {
                        var result = ' ';
            ...
            computeAbsoluteTrackBreadths: function() {
                ...
                var initializeFromConstraints = function(v) {
                    ...
                    return {
                        ...
                        limit: limit,
                        breadth: 0,
                        flags: (limit == infinity ? LIMIT_IS_INFINITE : 0) | 0,
                    };
                };
                ...
                //
                var distributeEquallyAmongTracks = function distributeEquallyAmongTracks(
                    xSizes,
                    kind,
                    tracks,
                    spaceToDistribute,
                    ...
                    if (kind == 'base') {
                        ...
                        if (enforceLimit) {
                            for (var t = tracks.length; t--; ) {
                                ...
                                if (
                                    enforceLimit &&
                                    xSizes[cx].flags & (LIMIT_IS_INFINITE == 0) &&
                                    newBase > xSizes[cx].limit
                                ) {
                ...
                //
                var computeTrackBreadth = function(
                    xSizes,
                    specifiedSizes,
                    getMinWidthOf,
                    getMaxWidthOf,
                    ...
                    for (var x = specifiedSizes.length; x--; ) {
                        ...
                        if (
                            specifiedSizes[x].minType == TRACK_BREADTH_MIN_CONTENT ||
                            specifiedSizes[x].minType == TRACK_BREADTH_AUTO
                        ) {
                        ...
                        // If the track has a max-content max track sizing function
                            ...
                            specifiedSizes[x].maxType == TRACK_BREADTH_MAX_CONTENT ||
                            specifiedSizes[x].minType == TRACK_BREADTH_AUTO
                        ) {
                            // Consider the items in it with a span of 1:
                            for (var i = this.items.length; i--; ) {
                    ...
                    for (var span = 2; items_done < this.items.length && span <= specifiedSizes.length; span++) {
                        ItemLoop: for (var i = this.items.length; i--; ) {
                            ...
                            var distributeFreeSpace = function(
                                requiredSpace,
                                kind /*'base'|'limit'*/,
                                target /*'min-content'|'max-content'*/
                            ) {
                                while (true) {
                                    ...
                                    if (spaceToDistribute <= 1 / 1024) {
                                        //due to double precision, this may never reach perfect 0
                                        for (var cx = item_xStart; cx < item_xEnd; cx++) {
                                            if (xSizes[cx].limit == infinity) {
                                    ...
                                    for (var cx = item_xStart; cx < item_xEnd; cx++) {
                                        rows_and_limits.push({
                                            ...
                                            minIsMinContent:
                                                specifiedSizes[cx].minType == TRACK_BREADTH_MIN_CONTENT ||
                                                specifiedSizes[cx].minType == TRACK_BREADTH_AUTO,
                                            ...
                                            maxIsMaxContent:
                                                specifiedSizes[cx].maxType == TRACK_BREADTH_MAX_CONTENT ||
                                                specifiedSizes[cx].maxType == TRACK_BREADTH_AUTO,
                                        });
                                    }
                ...
                var computeTrackBreadthIncrease = function(
                    xSizes,
                    specifiedSizes,
                    fullSize,
                    fullDistributableSize,
                    getMinWidthOf,
                    getMaxWidthOf,
                    getXStartOf,
                    getXEndOf
                ...
                var computeFlexibleTrackBreadth = function(
                    xSizes,
                    specifiedSizes,
                    fullSize,
                    fullDistributableSize,
                ...
                var computeFinalTrackBreadth = function(
                    xSizes,
                    this_xSizes,
                    fullWidth,
                ...
                var getMinWidthOf = function(item) {
                    return item.minWidth + item.hMargins - Math.max(0, item.xEnd - item.xStart - 1) * colGap;
                };
                var getMaxWidthOf = function(item) {
                    return item.maxWidth + item.hMargins - Math.max(0, item.xEnd - item.xStart - 1) * colGap;
                };
                ...
                var getMinHeightOf = function(item) {
                    return (
                        item.element.offsetHeight + item.vMargins - Math.max(0, item.yEnd - item.yStart - 1) * rowGap
                    );
                };
            ...
            generatePolyfilledStyle: function() {
                ...
                if (true) {
                    ...
                    for (var i = this.items.length; i--; ) {
                        ...
                        if (item.minWidth <= width || isReplaced.test(item.element.tagName)) {
                            // TODO: fix that... (should only do it for auto elements with stretch enabled)
                ...
                switch (item.specifiedXEnd.type) {
                    ...
                    case LOCATE_SPAN:
                        if (item.specifiedXEnd.name) {
                            ...
                            xEnd = this.findXLine(
                                item.specifiedXEnd.name,
                                xStart + 1,
                                (item.specifiedXEnd.index || 1) - 1
                            );
                        } else {
                            // Set the corresponding edge N lines apart from its opposite edge.
                ...
                switch (item.specifiedYEnd.type) {
                    ...
                    case LOCATE_SPAN:
                        if (item.specifiedYEnd.name) {
                            // Set the corresponding edge N lines apart from its opposite edge.
                            yEnd = this.findYLine(
                                item.specifiedYEnd.name,
                                yStart + 1,
                                (item.specifiedYEnd.index || 1) - 1
                            );
    ...
    !(function(window, document) {
        ...
        var enablePolyfill = function() {
            ...
            cssCascade.startMonitoringProperties(gridProperties, {
            ...
            cssCascade.startMonitoringProperties(gridItemProperties, {
                onupdate: function onupdate(element, rule) {
                    // log some message in the console for debug
                    cssConsole.dir({
                        message: 'onupdate',
